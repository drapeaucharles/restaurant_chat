...
def update_owner_profile(
    ...
    """
    Legacy endpoint - use PUT /profile instead.
    Update restaurant profile for authenticated owner.
    """
    # Prepare the data dictionary for the restaurant
    updated_data = {
        "name": payload.name,
        "story": payload.story,
        "menu": [item.dict() for item in payload.menu],
        "faq": [faq.dict() for faq in payload.faq] if payload.faq else [],
    }
    
    # Handle opening hours - convert to dict if provided
    if payload.opening_hours:
        updated_data["opening_hours"] = payload.opening_hours.dict(exclude_none=True)
