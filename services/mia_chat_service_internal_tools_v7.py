"""
MIA Chat Service V7 - Based on V5 with no_food_response tool
Adds greeting/goodbye handling without breaking existing functionality
"""
import os
import requests
import json
...
8. no_food_response - For greetings, goodbyes, thanks, general chat
   Parameters: response_type (greeting, goodbye, thanks, hours, general)
   PRIORITY: Use this for "hello", "bye", "see you", "thanks", "perfect" WITHOUT food context
...
RULES:
1. PRIORITY: For greetings/goodbyes/thanks, use no_food_response FIRST
2. Focus on CURRENT MESSAGE intent, not just conversation history
3. Select tools that best answer the customer's request
4. Include all necessary tools (e.g., allergy filter + category search)
5. For get_dish_details: use what the customer said (tool handles fuzzy matching)
6. For categories: use exact category names from the list above
7. For "seafood" requests: use "Seafood" category (includes both fish and shellfish)
8. Return JSON array with tool names and parameters
...
- "Thanks" → 
  [{{"tool": "no_food_response", "parameters": {{"response_type": "thanks"}}}}]
  
- "Hello!" → 
  [{{"tool": "no_food_response", "parameters": {{"response_type": "greeting"}}}}]
  
- "Perfect, see you soon!" → 
  [{{"tool": "no_food_response", "parameters": {{"response_type": "goodbye"}}}}]
...
def build_phase2_prompt(message: str, tool_results: List[Dict], restaurant_name: str, 
    ...
    non_food_tools_executed = any(
        result.get("tool") in ["explain_reasoning", "handle_misunderstanding", "restaurant_info", "change_preference", "no_food_response"]
        and result.get("do_not_suggest_food", False)
        for result in tool_results
    )
    
    # Check for no_food_response specifically
...
def generate_response_internal_tools_v7(req: Any, db: Session) -> Any:
...
def mia_chat_service_internal_tools_v7(req: Any, db: Session) -> Any:
    return generate_response_internal_tools_v7(req, db)