"""
MIA Chat Service - Uses MIA backend instead of OpenAI
Provides the same interface as chat_service.py but uses MIA for responses
"""
import requests
import re
...
def get_mia_response(prompt: str, max_tokens: int = 150) -> str:
    """
    Get response from MIA backend
    ...
    """
    try:
        # First, get work from MIA backend
        work_response = requests.get(
            f"{MIA_BACKEND_URL}/get_work",
            params={"miner_id": "restaurant-service"},
            timeout=5
        )
        
        if work_response.status_code != 200:
            logger.warning(f"No work available from MIA: {work_response.status_code}")
            # Fallback to direct generation if available
            return generate_direct_response(prompt, max_tokens)
        
        work = work_response.json()
        request_id = work.get("request_id")
        
        # Submit the prompt as work result
        result_data = {
            "miner_id": "restaurant-service",
            "request_id": request_id,
            "result": {
                "response": prompt,  # Send our prompt
                "tokens_generated": 0,
                "processing_time": 0
            }
        }
        ...
        if result_response.status_code == 200:
            return result_response.json().get("response", "I'm having trouble understanding. Could you please rephrase?")
        else:
            return generate_direct_response(prompt, max_tokens)
...
def generate_direct_response(prompt: str, max_tokens: int = 150) -> str:
    """
    Generate response directly from a MIA miner if available
    
    This would connect to a local or dedicated MIA miner instance
    """
    try:
        # Try to connect to a local MIA miner instance
        local_miner_url = os.getenv("MIA_MINER_URL", "http://localhost:8000")
        
        response = requests.post(
            f"{local_miner_url}/generate",
            json={
                "prompt": prompt,
        ...
        if response.status_code == 200:
            return response.json().get("text", "I'm having trouble understanding. Could you please rephrase?")
        else:
            logger.error(f"Local miner error: {response.status_code}")
            return "I'm temporarily unavailable. Please try again in a moment."
            
    except Exception as e:
        logger.error(f"Error with local miner: {e}")
        return "I'm temporarily unavailable. Please try again in a moment."
...
def mia_chat_service(req: ChatRequest, db: Session) -> ChatResponse:
    ...
    try:
        ...
        answer = get_mia_response(full_prompt, max_tokens=150)
