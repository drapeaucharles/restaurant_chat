"""
Working memory RAG service - Simple but effective
This version ensures memory actually works across messages
"""
import logging
...
class WorkingMemoryRAG:
    """Simple RAG with working memory"""
    ...
    def get_memory_key(self, restaurant_id: str, client_id: str) -> str:
        """Get memory key"""
        return f"memory:{restaurant_id}:{client_id}"
    ...
    def __call__(self, req: ChatRequest, db: Session) -> ChatResponse:
        """Process chat with working memory"""
        
        # Get existing memory FIRST
        memory = self.get_memory(req.restaurant_id, req.client_id)
        logger.info(f"Retrieved memory for {req.client_id}: name={memory.get('name')}, history={len(memory.get('history', []))}")
        ...
        if not any(word in message_lower for word in ['hi', 'hello', 'hey']) or len(message_lower) > 20:
            try:
...
Response:"""
        ...
        logger.info(f"Saved memory for {req.client_id}: name={memory.get('name')}")
...
# Create singleton
working_memory_rag = WorkingMemoryRAG()