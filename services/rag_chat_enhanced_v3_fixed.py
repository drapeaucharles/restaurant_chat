"""
Enhanced RAG Chat Service v3 with FIXED memory usage
This version properly uses stored memory in responses
"""
import logging
...
class EnhancedRAGChatV3Fixed:
    """Enhanced RAG with FIXED conversation memory usage"""
    ...
    def __call__(self, req: ChatRequest, db: Session) -> ChatResponse:
        """Process chat request with FIXED conversation memory"""
        
        logger.info(f"Enhanced RAG v3 Fixed - Processing: '{req.message[:50]}...'")
        ...
        # Create ENHANCED system prompt that EXPLICITLY uses memory
        persona_name = get_persona_name(language)
        
        # Build prompt based on what we know
        if customer_info['name']:
            if "my name" in req.message.lower() or "call me" in req.message.lower():
                system_prompt = f"""You are {persona_name}, a friendly AI assistant for {business_name}.

The customer's name is {customer_info['name']}. They just asked about their name or how you should address them.
IMPORTANT: Acknowledge that you know their name is {customer_info['name']} and that you'll use it when talking to them.
Be warm and personal in your response."""
            elif "what is my name" in req.message.lower() or "remember my name" in req.message.lower():
                system_prompt = f"""You are {persona_name}, a friendly AI assistant for {business_name}.

The customer's name is {customer_info['name']}. They're asking if you remember their name.
IMPORTANT: Confirm that yes, you know their name is {customer_info['name']}.
Be friendly and conversational."""
            else:
                system_prompt = f"""You are {persona_name}, a friendly AI assistant for {business_name}.

You are talking to {customer_info['name']}. Use their name naturally in your response when appropriate.
...
enhanced_rag_chat_v3_fixed = EnhancedRAGChatV3Fixed()