...
def format_menu_for_context(menu_items, query):
    """Format menu items for context - simplified version"""
    if not menu_items:
    ...
    # Simple approach: Always search for relevant menu items based on the query
    # Let the AI decide if/how to use them
    found_items = []
    ...
    for item in menu_items:
        ...
        for word in query_words:
            if (word in name_lower or 
                ...
                any(word in str(ing).lower() for ing in ingredients)):
                relevant = True
                break
    ...
    # If items found, format them nicely
    if found_items:
        # Group by category
        by_category = {}
        for item in found_items[:20]:  # Reasonable limit
            cat = item['category']
    ...
    # Return the context (empty string if nothing relevant found)
    return "\n".join(context_lines)
        found_items = []
