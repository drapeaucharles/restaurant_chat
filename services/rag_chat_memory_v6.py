"""
Memory Working V6 - Adding extract_and_update_memory
Testing if complex extraction breaks the service
"""
import logging
...
class WorkingMemoryRAGV6:
    """Working memory + All features + extract_and_update_memory"""
    ...
    def get_memory_key(self, restaurant_id: str, client_id: str) -> str:
        """Get memory key"""
        return f"memory_v6:{restaurant_id}:{client_id}"
    ...
    def extract_and_update_memory(self, memory: Dict, message: str, response: str):
        ...
        # Extract name (redundant but kept for consistency)
        name_match = re.search(r'my name is (\w+)', message, re.IGNORECASE)
        if name_match:
            memory['name'] = name_match.group(1).capitalize()
            logger.info(f"V6: Captured name in extract: {memory['name']}")
        
        # Extract dietary restrictions (redundant but more comprehensive)
        for restriction in ['vegetarian', 'vegan', 'gluten-free', 'dairy-free', 'nut-free', 'kosher', 'halal']:
            if restriction in message_lower:
                if restriction not in memory.get('dietary_restrictions', []):
                    memory.setdefault('dietary_restrictions', []).append(restriction)
                    logger.info(f"V6: Added dietary restriction in extract: {restriction}")
        
        # Extract preferences from message
        if 'i like' in message_lower or 'i love' in message_lower or 'favorite' in message_lower:
            # Store just the preference part, not the whole message
            pref_match = re.search(r'(i (?:like|love)|favorite[s]?(?:\s+is)?)\s+(.+?)(?:\.|!|$)', message_lower)
            if pref_match:
                preference = pref_match.group(2).strip()
                if preference and preference not in memory.get('preferences', []):
                    memory.setdefault('preferences', []).append(preference)
                    logger.info(f"V6: Added preference: {preference}")
        
        # Extract mentioned items from response
        if '$' in response:
            items = re.findall(r'([^$\n]+?)\s*\(\$[\d.]+\)', response)
            for item in items:
                item = item.strip()
                if item and len(item) < 100 and item not in memory.get('mentioned_items', []):
                    memory.setdefault('mentioned_items', []).append(item)
                    logger.info(f"V6: Added mentioned item: {item}")
        
        # Extract topics from message
        topic_keywords = ['menu', 'hours', 'location', 'delivery', 'reservation', 'special', 'event', 'dessert', 'drink', 'appetizer']
        for topic in topic_keywords:
            if topic in message_lower and topic not in memory.get('topics', []):
                memory.setdefault('topics', []).append(topic)
                logger.info(f"V6: Added topic: {topic}")
    ...
    def __call__(self, req: ChatRequest, db: Session) -> ChatResponse:
        ...
        logger.info(f"V6: Retrieved memory for {req.client_id}: name={memory.get('name')}, history={len(memory.get('history', []))}")
        
        # Extract name from current message (do this before extract_and_update)
        name_match = re.search(r'my name is (\w+)', req.message, re.IGNORECASE)
        if name_match:
            memory['name'] = name_match.group(1).capitalize()
            logger.info(f"V6: Captured name: {memory['name']}")
        
        # Classify query type
        try:
            query_type = HybridQueryClassifier.classify(req.message)
            language = detect_language(req.message)
            logger.info(f"V6: Query type: {query_type.value}, Language: {language}")
        except Exception as e:
            logger.error(f"V6: Classification failed: {e}")
        ...
        # Check for dietary mentions (before extract_and_update)
        ...
        for diet in dietary_keywords:
            if diet in message_lower and diet not in memory.get('dietary_restrictions', []):
                ...
                logger.info(f"V6: Added dietary restriction: {diet}")
        ...
        if query_type not in [QueryType.GREETING] and len(req.message) > 10:
            try:
                if is_allergen_query:
                    ...
                    logger.info(f"V6: Using allergen service for query")
                ...
                logger.warning(f"V6: Menu search failed: {e}")
        ...
        # Use context formatter to build the prompt
...
Response:"""
        ...
        try:
            ...
            logger.info(f"V6: Using params for {query_type.value}: {params}")
        except Exception as e:
            logger.error(f"V6: Failed to get params: {e}")
            params = {
                'max_tokens': 200,
                'temperature': 0.7
            }
        ...
        # Validate response
        try:
            logger.info(f"V6: Validating response...")
            validated_answer = response_validator.validate_and_correct(answer, db, req.restaurant_id)
            if answer != validated_answer:
                logger.info(f"V6: Response was modified by validator")
            answer = validated_answer
        except Exception as e:
            logger.error(f"V6: Response validation failed: {e}")
        
        # NEW: Extract and update memory from this exchange
        try:
            self.extract_and_update_memory(memory, req.message, answer)
            logger.info(f"V6: Extracted and updated memory")
        except Exception as e:
            logger.error(f"V6: extract_and_update_memory failed: {e}")
        ...
        logger.info(f"V6: Saved memory for {req.client_id}: name={memory.get('name')}, dietary={memory.get('dietary_restrictions', [])}, preferences={len(memory.get('preferences', []))}, topics={memory.get('topics', [])}")
...
# Create singleton
working_memory_rag_v6 = WorkingMemoryRAGV6()